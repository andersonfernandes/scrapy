...
def build_component_list(compdict: dict, custom: Optional[dict] = None, convert: Callable = update_classpath) -> list:
...
def arglist_to_dict(arglist: list) -> dict:
...
def init_env(project: str = "default", set_syspath: bool = True):
...
def feed_complete_default_values_from_settings(feed: dict, settings: Settings) -> dict:
    out = feed.copy()
