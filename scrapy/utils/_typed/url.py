...
from w3lib.url import add_or_replace_parameter, any_to_uri
from scrapy.spiders import Spider
from typing import Optional

def url_is_from_any_domain(url: str, domains: list) -> bool:
...
def url_is_from_spider(url: str, spider: Spider) -> bool:
...
def url_has_any_extension(url: str, extensions: list) -> bool:
    """Return True if the url ends with one of the extensions provided"""
    lowercase_path = parse_url(url).path.lower()
    return any(lowercase_path.endswith(ext) for ext in extensions)


def parse_url(url: str, encoding: Optional[str] = None) -> ParseResult:
